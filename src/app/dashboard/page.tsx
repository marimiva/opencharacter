...
async function getMessageData() {
  ...
  const messageCounts = await Promise.all(userCharacters.map(async (character) => {
    ...
    const filledData = dateArray.map(date => {
      ...
      return { date, count: matchingCount ? matchingCount.count : 0 };
    });
...
async function getRecentMessages() {
  ...
  recentSessions.forEach(session => {
    if (uniqueSessions.has(session.id)) return;
  ...
  return Array.from(uniqueSessions.values()).sort((a, b) => b.timestamp - a.timestamp);
}
...
export default async function Dashboard() {
  ...
  return (
    <div className="md:ml-16 text-white md:p-4 mb-16 flex flex-col gap-4">
      ...
      <div className="w-full">
        {userCountData && userCountData.length > 0 && (
