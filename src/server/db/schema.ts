...
export const socialSubmissions = sqliteTable("social_submission", {
  ...
  createdAt: integer("createdAt", { mode: "timestamp_ms" }).notNull().$defaultFn(() => sql`CURRENT_TIMESTAMP`),
});
...
export const characters = sqliteTable("character", {
  ...
  createdAt: integer("createdAt", { mode: "timestamp_ms" }).notNull().$defaultFn(() => sql`CURRENT_TIMESTAMP`),
  updatedAt: integer("updatedAt", { mode: "timestamp_ms" }).notNull().$defaultFn(() => sql`CURRENT_TIMESTAMP`),
...
export const chat_sessions = sqliteTable("chat_session", {
  ...
  last_message_timestamp: integer("last_message_timestamp", { mode: "timestamp_ms" }).notNull().$defaultFn(() => sql`CURRENT_TIMESTAMP`),
  created_at: integer("created_at", { mode: "timestamp_ms" }).notNull().$defaultFn(() => sql`CURRENT_TIMESTAMP`),
  updated_at: integer("updated_at", { mode: "timestamp_ms" }).notNull().$defaultFn(() => sql`CURRENT_TIMESTAMP`),
...
export const group_chat_session_characters = sqliteTable("group_chat_session_characters", {
  id: text("id").primaryKey().$defaultFn(() => crypto.randomUUID()),
  sessionId: text("sessionId").notNull().references(() => group_chat_sessions.id, { onDelete: "cascade" }),
  characterId: text("characterId").notNull().references(() => characters.id, { onDelete: "cascade" }),
  createdAt: integer("createdAt", { mode: "timestamp_ms" }).notNull().$defaultFn(() => sql`CURRENT_TIMESTAMP`),
});
